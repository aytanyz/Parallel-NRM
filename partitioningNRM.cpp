...
class NotificationQueue //queue
{
    public:
        double _time;
        pair<int, int> _shared_x;        // ??????????? should solve how to define the length of shared elements
        //pair(id_of_shared_element, amount_of_this_shared_elemet)
};
...
/*
void updateSpecies(int fired_reaction)
{
	int idOfElement;
	int changedAmount;
...
void nextReactionMethods( int i_th, SubNetwork *object, SubNetwork *shared,
	...
	/*	
	for(int i=0; i<object->reactions.size(); i++)
		cout<<object->reactions[i]<<" ";
		cout<<endl; */

     int n = object->reactions.size();
     int n_shared_species = shared->species.size();
/*    cout<<i_th<<" thread "<<endl;
	cout<<"n_shared_species="<<n_shared_species<<endl; */
	...
	for(int i=0; i<n; i++)
	{
      ...
      /*   cout<<i_th<<" thread "<<endl;
		cout<<"proponsity for reaction "<<object->reactions[i]<<" : "<<my_p[i]<<endl; */
	}	
	...
	for(int i=0; i<n; i++)
	{
       ...
       /*  cout<<i_th<<" thread "<<endl;
		cout<<"tau for reaction "<<object->reactions[i]<<" : "<<my_tau[i]<<endl; */
	}
	...
	//for(int it=0; it<50; it++)
	{
		...
		/*
		cout<<i_th<<" thread: TAU ";
		for(int i=0; i<n; i++)
		  cout<<my_tau[i]<<"  "<<endl;
		cout<<endl; */
		
		/* 
		cout<<i_th<<" thread: Heap ";
		for(int i=0; i<n; i++)
		  cout<<my_queue.keyOf(i)<<"  "<<endl;
		cout<<endl; */
		...
		int fired_reaction = object->reactions[min_tau_index];
       	...
       	cout<<" THREAD ["<<i_th<<"]: ";
		cout<<"Choosen reaction="<<fired_reaction<<" and tau="<<min_tau<<endl;
...
//---------------------------------------------------------------------------------------------
        ...
        if(!notification_for_this_thread.empty())
        {
            ...
            if(temp_notification_for_this_thread._time < t)
            {
				...
				if(stack_of_shared_species[my_id].empty() || temp_notification_for_this_thread._time > t-min_tau)  
				{
					// we should change the abundance of shared elements as it was given on the notification of this thread
					cout<<" THREAD ["<<i_th<<"]: ";
					//cout<<"Change on the shared specie which is: "<<idOfElement<<endl;
					cout<<"So updating abundances as in other thread when t = ";
                    cout<<temp_notification_for_this_thread._time<<endl;
				...
				else
				{
					while(!stack_of_shared_species[my_id].empty())
					{
						...
						if(!notification_for_other_thread.empty() && temp_notification_for_this_thread._time > notification_for_other_thread.front()._time)
						{
							...
							//cout<<"Other thread will do undo!"<<endl;
							break;
						}
						else if(temp_stack_of_shared_species.time_when_changes_happend > temp_notification_for_this_thread._time)
						{
							undo_reaction = true;
							//other thread did chage before this thread. So we should undo and return to the safe state
							cout<<" THREAD ["<<i_th<<"]: ";
                            cout<<" should do undo!"<<endl;
							//cout<<"other thread did chage before this thread. So we should undo and return to the safe state"<<endl;
							stack_of_shared_species[my_id].pop();							
							...
							while(!notification_for_other_thread.empty() && notification_for_other_thread.back()._time > t)
							{
								...
								//cout<<" THREAD ["<<i_th<<"]: ";
								//cout<<"Deleting note for other thread"<<endl; 
							}
...
//---------------------------------------------------------------------------------------------
        ...
        if(t<ENDTIME && this_thread_reached_t_end)
        {
            ...
            cout<<" THREAD ["<<i_th<<"]: ";
            cout<<"no more reched end time!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
        }
        else if(t-min_tau>ENDTIME && this_thread_reached_t_end)
        {
            t -= min_tau;
            cout<<" THREAD ["<<i_th<<"]: ";
            cout<<"waiting for other thread to finish @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;
            continue;
        }
		...
		if(change_on_shared_species == true)
		{
            ...
            temp_stack_of_shared_species.safe_time = t - min_tau;
			for(int i=0; i<M; i++)
		...
		//cout<<"Tecomputing tau and propensity"<<endl;
		...
		cout<<" THREAD ["<<i_th<<"]: ";
		cout<<"ending "<<iter<<endl;

		if(t>ENDTIME)
		{
			this_thread_reached_t_end = true;
			cout<<" THREAD ["<<i_th<<"]: ";
			cout<<"reached end time%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"<<endl;
		}
	}
...
//Starting our simulation
	auto startReac = chrono::high_resolution_clock::now();
		//cout<<endl<<"Calling NRM for Alpha !"<<endl;
		//nextReactionMethods(1, ref(alpha), ref(shared), ref(notify_alpha), ref(notify_beta));
		...
		//cout<<endl<<"Calling NRM for BETA !"<<endl;
		//nextReactionMethods(2, ref(beta), ref(shared), ref(notify_beta), ref(notify_alpha));
